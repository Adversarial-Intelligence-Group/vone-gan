...
def get_model(model_arch, n_classes, channels, latent_dim, image_size=224, **kwargs):
    ...
    if model_arch.lower() == 'acgan':
        ...
        print('Model: robgan')
        discriminator = robgan.Discriminator(out_ch=channels, ch=16, n_classes=n_classes)
        generator = robgan.Generator(ch=16, dim_z=latent_dim, n_classes=n_classes, bottom_width=4) # just 4
    else:
    ...
    if model_arch.lower() == 'robgan':
        bottleneck = nn.Conv2d(256, 32, kernel_size=1, stride=1, bias=False)
    else:
        bottleneck = nn.Conv2d(256, 16, kernel_size=1, stride=1, bias=False)
